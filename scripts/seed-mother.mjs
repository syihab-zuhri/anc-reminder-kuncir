...
async function seedMother() {
  ...
  try {
    ...
    const planRes = await client.query("SELECT id FROM anc_plan_versions WHERE status = $1", ["ACTIVE"]);
    const planId = planRes.rows[0].id;

    // Check if mother already exists
    const motherCheck = await client.query("SELECT id, full_name FROM mothers WHERE full_name = $1", ["Siti Aminah"]);
    ...
    if (!motherId) {
      ...
      await client.query(
        ...
        [motherId, hcId, villageId, "Siti Aminah", dummyNikCipher, "Dusun Krajan RT 02 RW 01, Desa Kuncir", "081234567890"]
      );
      ...
      // Milestone schedule
      const rules = await client.query("SELECT id, code, target_week_start, target_week_end FROM anc_milestone_rules WHERE plan_version_id = $1", [planId]);
    ...
    const lookupHash = crypto.createHmac("sha256", motherSecret)
